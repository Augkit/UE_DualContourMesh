	...
	TArray<uint8>& OutSamples, FText& OutError) const
{
	...
	const FVectorInt SampleDimensions(Target->CellCount.X + 1, Target->CellCount.Y + 1, Target->CellCount.Z + 1);
	const int64 SampleCount = static_cast<int64>(SampleDimensions.X) * SampleDimensions.Y * SampleDimensions.Z;
	if (SampleCount > MAX_int32)
	{
		OutError = NSLOCTEXT("VolumeSampler", "TargetTooLarge", "The target DualContour sample count exceeds TArray capacity.");
		return false;
	}
	if (!Prepare(OutError))
		return false;
	...
	for (int32 Z = 0; Z < SampleDimensions.Z; ++Z)
		for (int32 Y = 0; Y < SampleDimensions.Y; ++Y)
			...
			{
				const FVector TargetPosition = Target->GetSampleLocalPosition(X, Y, Z);
